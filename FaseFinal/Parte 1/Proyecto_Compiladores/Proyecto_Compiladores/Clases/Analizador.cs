...
namespace Proyecto_Compiladores.Clases
{
    class Analizador
    {
        ...
        public String MsjoError, Nombre_Compi, Nombre_Accion;
        ...
        public static List<Token> Lista_Tokens;
        public static List<No_Terminales> Lista_NT;
        List<string> Lista_Alias_Coments;
        public static List<string> Lista_KeyWords;
        public static List<KeyWords> Lista_KeyWords_NEW;
        public static List<Producciones> Lista_Producciones;
        public static List<string> Lista_Acciones;
        ...
        public Analizador()
        {
            ...
            Lista_KeyWords_NEW = new List<KeyWords>();
            Lista_Acciones = new List<string>();
        }
                ...
                if (texto[pos] == '\"')
                {
                    ...
                    if (texto[pos] == '\"')
                    {
                        if (EsPalabra(palabra))
                        {
                            ...
                            KeyWords nuevaKey = new KeyWords(temporal_token + 1, '\'' + palabra.ToLower() + '\'');
                            Lista_KeyWords_NEW.Add(nuevaKey);
                            pos = OmitirEspacios(texto, pos + 1);
        ...
        private bool Validar_Produccion(string texto, int tem, string ntActual)
        {
            ...
            else if (texto[tem] == '{')
            {
                if (Otro_Analisis_Produccion(texto, tem))
                {
                    ...
                    Nombre_Accion = palabritra; //Se mete a una variable para poder guardarla a la lista de acciones
                    tem = OmitirEspacios(texto, tem + palabritra.Length);
                    if (texto[tem] == '}')
                    {
                        //int posTempo =  OmitirEspacios(texto, tem + 1);
                        tem = OmitirEspacios(texto, tem + 1);
                        if (texto[tem/*posTempo*/] == '.')
                        {
                            Lista_Acciones.Add(Nombre_Accion.ToLower()); //Se guarda
                            Nombre_Accion = ""; //Se limpia
                        ...
                        return Manejo_Error("Se esperaba un '.' o '|' después de la llave que cierra.", PosActual + tem - palabritra.Length);
                    }
